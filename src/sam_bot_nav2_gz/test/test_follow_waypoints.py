        ...
        while time.time() - start_time < timeout:
            try:
                ...
                if self.process.poll() is not None:
                    print(f"[{self.name}]: Process exited with code {self.process.poll()}")
...
def navigation_stack():
    ...
    launch_cmd = [
        "ros2", "launch", "sam_bot_nav2_gz", "waypoint_follower_example_launch.py",
        f"world_file:={world}", "run_headless:=True"
    ]
    
    # Create monitored process
    nav_process = MonitoredProcess(
        cmd=launch_cmd,
        name="Navigation"
    )
...
#     
#     # Create monitored process
#     odometry_process = MonitoredProcess(
#         cmd=odometry_cmd,
#         name="OdometryTest"
#     )
#     
...
# def test_create_artifacts(navigation_stack, rosbag_data):
#     """Generate charts and extract media from the rosbag after test completion."""
#     # Make sure navigation tests have completed
#     # We could add a specific check or just rely on test order
    
#     # Ensure processes are stopped
#     if navigation_stack["process"].poll() is None:
#         navigation_stack["process"].terminate()
#         navigation_stack["process"].wait(timeout=10)
