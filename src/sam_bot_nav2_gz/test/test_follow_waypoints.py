...
def navigation_stack():
    ...
    return launch.actions.ExecuteProcess(
        ...
        cached_output=True,
        output='both',        
        sigterm_timeout="600.0",  # Wait 30 seconds after SIGTERM before SIGKILL
        sigkill_timeout="600.0",
    )

# This function specifies the processes to be run for our test.
@launch_pytest.fixture(scope="module") # Set the scope so the processes are not killed after each test
def launch_description(navigation_stack):
...
def test_nav2_ready(navigation_stack, launch_context):
    def validate_output(output):
        assert "Nav2 is ready for use!" in output, 'process never printed "Nav2 is ready for use!"'
...
def test_no_skipped_waypoints(navigation_stack, launch_context):
    ...
    def validate_output(output):
        assert "Executing current waypoint: 1" in output, 'process never printed "Executing current waypoint: 1/1"'
    process_tools.assert_output_sync(
        launch_context, navigation_stack, validate_output, timeout=250)
